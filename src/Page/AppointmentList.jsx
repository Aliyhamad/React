...
const AppointmentList = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/api/v7/appointment')
      .then(response => {
  ...
  const handleAddAppointment = () => {
    navigate('/Main/appointment_update');
  };

  const handleUpdateAppointment = (id) => {
    navigate(`/Main/appointment_update/${id}`);
  };

  const handleDeleteAppointment = (id) => {
    axios.delete(`http://localhost:8080/api/v7/appointment/${id}`)
      .then(response => {
  ...
  return (
    <div className="appointment-table-container">
      ...
      <button className="add-appointment" onClick={handleAddAppointment}>Add Appointment</button>
